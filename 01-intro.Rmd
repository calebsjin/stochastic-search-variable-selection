...
Let $\uD=(\uM,\uy)$ be a dataset, where $\uM\in \mathbb{R}^{n\times p}$ and $\uy\in \mathbb{R}^{n}$ with $n=100$ and $p=500$. We are interested in finding predictors related to the response.
Consider a high dimensional linear regression model as follows:
\begin{eqnarray}
\uy=\uX\ubeta + \uepsilon
(\#eq:1)
\end{eqnarray}
where $\uy=(y_1,y_2,\ldots,y_{n})^{\T}$ is the $n$-dimensional response vector, $\uX=[1,\uM]=[\ux_0, \ux_1,\ldots,\ux_{p}]$ is the $n\times (p+1)$ design
matrix, and $\uepsilon \sim \mathcal{N}_{n}(\0,\sigma^2\uI_{n})$. Note that as $p+1>n$, $\uX$ is not full rank.
